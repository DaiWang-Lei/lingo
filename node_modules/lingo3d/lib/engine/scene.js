...
const textureCubeRenderTargetMap = new WeakMap();
createEffect(() => {
    ...
    if (skybox) {
        if (Array.isArray(skybox))
            ...
            loadTextureAsync(skybox).then(texture => {
                if (done)
                    return;
                const renderTarget = forceGet(textureCubeRenderTargetMap, texture, () => (new WebGLCubeRenderTarget(texture.image.height)));
